...
def summarize_neighborhood(gmgc, neighborhood, orf_data, n_contig):
    ...
    for pos, l in neighborhood.items():
        ...
        if (len(l) > 0):
            ...
            if (most_freq[0] != "NA"):
                neigh_data['strand'] = orf_data[pos][most_freq[0]]['strand']
...
def format_neighborhood(gmgc,
    ...
    for dummy, v in neighborhood_genes.items():
        ...
        if gmgc in central_gene:
            ...
            for n in range(0,(neighbor_range*2+1),1):
                ...
                if gmgc_orf == '':
                    gmgc_orf = 'NA'
                    # if gmgc == "135_757_792" and pos == -4:
                        # print(retrieve_gene_info(gene_list[n],
                                                # coll_unigenes))
                else:
    ...
    if retrieve_unique_contigs:
        ...
        for cont, n in dict(Counter(contigs)).items():
            ...
            contig[0]['KEGG'] = subject_functional_data(gmgc,
                                                     neighborhood_genes,
                                                     neighbor_range,
                                                     "kegg")
            contig[0]['eggNOG'] = subject_functional_data(gmgc,
                                                     neighborhood_genes,
                                                     neighbor_range,
                                                     "Egg")
            unique_contigs[inc] = {}
            unique_contigs[inc]['neighborhood'] = contig
            unique_contigs[inc]['unigene'] = gmgc
            inc += 1
...
def neighbor_analysis(gmgc, unique_contigs=False, json=True):
    ...
    if not unique_contigs:
        data['KEGG'] = subject_functional_data(gmgc,
                                                 neighborhood_genes,
                                                 neighbor_range,
                                                 "kegg")
        data['eggNOG'] = subject_functional_data(gmgc,
                                                 neighborhood_genes,
                                                 neighbor_range,
                                                 "Egg")
        if (data['KEGG']):
            neighborhood[0]['KEGG'] = data['KEGG']
        if (data['eggNOG']):
            neighborhood[0]['eggNOG'] = data['eggNOG']
...
def launch_analysis(query, n_range, cutoff, cluster=True):
    ...
    if cluster:
        ...
        unigene_list = get_unigenes_and_tree(query, "data/", client)
    ...
    with open('data/eggNOG_LEVELS.txt', 'w') as handle:
        dump(egg_levels, handle)
    with open('data/' + query + '.txt', 'w') as handle:
